...
pub struct Command<const N: usize> {
    ...
    pub parameter: Option<[u8; N]>, // pub parameter: Option<Box<[u8]>>,
...
impl Commands {
    ...
    pub fn command<const N: usize>(&self) -> Command<N> {
        match self {
            Commands::OperatingChannel(channel) => Command::<N> {
                command: self.identifier(),
                parameter: Some([*channel as u8;N]),
                carriage_returns: 1,
            },
...
impl From<Commands> for Command<1> {
    fn from(value: Commands) -> Self {
        // Assert is correct variant??
        let ch = value as Commands::OperatingChannel;

        Self {
            command: ch.identifier(),
            parameter: Some([*ch as u8]),
            carriage_returns: 1,
        }
    }
}
